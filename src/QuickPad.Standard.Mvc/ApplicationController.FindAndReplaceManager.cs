...
{
    internal static class FindAndReplaceManager<TStorageFile, TStream>
        where TStream : class
    {
        public static (string text, string match, int start, int length)[] ReplaceAll(
            SettingsViewModel<TStorageFile, TStream> settings, string text, DocumentViewModel<TStorageFile, TStream> viewModel)
        {
            var count = 0;
            var offset = -1;
        ...
        public static (string text, string match, int start, int length) ReplaceNext(
            SettingsViewModel<TStorageFile, TStream> settings, string text, int offset, DocumentViewModel<TStorageFile, TStream> viewModel)
        {
        ...
        public static (string text, string match, int start, int length) SearchPrevious(SettingsViewModel<TStorageFile, TStream> settings,
            string text, int offset, DocumentViewModel<TStorageFile, TStream> viewModel)
        {
        ...
        public static (string text, string match, int start, int length) SearchNext(SettingsViewModel<TStorageFile, TStream> settings,
            string text, int offset, DocumentViewModel<TStorageFile, TStream> viewModel)
        {
            if (string.IsNullOrWhiteSpace(viewModel.FindAndReplaceViewModel.SearchPattern)) return (text, null, -1, 0);
        ...
        private static (string text, string match, int start, int length) Search(
            SettingsViewModel<TStorageFile, TStream> settings, string searchable, string pattern, bool useRegex, bool matchCase,
            SearchDirection direction)
        {
            ...
            else
            {
                ...
                var match = direction switch
                {
                    ...
                    SearchDirection.Backward => regex.Matches(searchable).Cast<Match>().LastOrDefault(),
                    _ => Match.Empty
                };
